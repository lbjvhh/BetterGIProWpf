    ...
    def do_POST(self):
        try:
            ...
            if self.path == "/yolo":
                ...
                with _lock: self._json(200, yolo_infer(img))
            elif self.path == "/ocr":
                img = Image.open(io.BytesIO(base64.b64decode(raw["image"]))).convert("RGB")
                with _lock: self._json(200, ocr_infer(img))
            elif self.path == "/whisper":
                wav = base64.b64decode(raw["audio"])
                with _lock: self._json(200, whisper_infer(wav))
            else: self._json(404, {"e": "nf"})
...
if __name__ == "__main__":
    ...
    load_yolo(); load_ocr(); load_whisper()
