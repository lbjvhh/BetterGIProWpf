"""
生成 5 类基准回归测试视频（跑图/战斗/采集/对话/传送）。
"""
import cv2, numpy as np
from pathlib import Path

OUT = Path(r"nitrogen_bridge/test_videos")
OUT.mkdir(parents=True, exist_ok=True)
W, H, FPS, SECONDS = 1280, 720, 15, 8
FOURCC = cv2.VideoWriter_fourcc(*"mp4v")

def make(name, scenes):
    path = OUT / f"{name}.mp4"
    vw = cv2.VideoWriter(str(path), FOURCC, FPS, (W, H))
    per = SECONDS * FPS // len(scenes)
    for text, bg in scenes:
        for _ in range(per):
            img = np.full((H, W, 3), bg, dtype=np.uint8)
            cv2.putText(img, text, (80, 360), cv2.FONT_HERSHEY_SIMPLEX,
