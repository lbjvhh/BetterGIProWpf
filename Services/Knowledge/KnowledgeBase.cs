...
public class KnowledgeBase
{
    ...
    {
        EnsureIndex();
        var qv = Vectorize(Tokenize(query)); var qm = Magnitude(qv);
        var hits = new List<SearchHit>();
        lock (_lock) foreach (var e in _entries)
        {
    ...
    {
        var wps = new List<object>(); var i = 0;
        foreach (var e in entries) wps.Add(new { id = i++, type = "teleport", pos = new[] { 0, 0 }, description = e.TaskDescription });
        return System.Text.Json.JsonSerializer.Serialize(new { waypoints = wps }, new System.Text.Json.JsonSerializerOptions { WriteIndented = true });
    }
    public string ExportJson() { lock (_lock) return System.Text.Json.JsonSerializer.Serialize(_entries); }
    public int ImportJson(string json) { var items = System.Text.Json.JsonSerializer.Deserialize<List<KnowledgeEntry>>(json) ?? new(); AddRange(items); return items.Count; }
    private static IEnumerable<string> Tokenize(string text) => text.ToLowerInvariant().Split(new[] { ' ', '，', '。', '、', ' ', '\n' }, StringSplitOptions.RemoveEmptyEntries).Where(w => w.Length >= 2);
    ...
    {
        foreach (var (k, v) in filters) { var f = k switch { "location"=>e.Location, "taskType"=>e.TaskType, "boss"=>e.BossName ?? "", "character"=>e.Character ?? "", "source"=>e.VideoSource, _=>"" }; if (!f.Contains(v, StringComparison.OrdinalIgnoreCase)) return false; }
        return true;
    }
}
