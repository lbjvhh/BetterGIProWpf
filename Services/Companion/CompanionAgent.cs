...
public class CompanionAgent
{
    ...
    public void TakeoverControl()
    {
        ...
        Log?.Invoke("玩家接管，AI 队友已停止并切换跟随");
    }

    public void SwitchMode(BehaviorMode mode) { Mode = mode; Log?.Invoke($"模式 → {mode}"); }
    ...
    private async Task RunLoopAsync()
    {
        while (_taskQueue.TryDequeue(out var task))
        {
            ...
            try
            {
                ...
                if (ok) { CompletedCount++; Log?.Invoke($"✔ 完成: {task.Goal}"); }
                else { FailedCount++; Log?.Invoke($"✘ 失败: {task.Goal}"); }
            }
            catch (Exception ex) { FailedCount++; Log?.Invoke($"✘ 异常: {ex.Message}"); }
            finally { CurrentTask = null; }
        }
    }

    private bool ExecuteFallback(CompanionTask task) { Log?.Invoke($"[内置] {task.Type}"); return true; }
    ...
    public static ParsedCommand ParseCommand(string text)
    {
        ...
        if (ContainsAny(t, "跟随", "跟着我")) return new ParsedCommand(CompanionTaskType.Follow, target, t);
        if (ContainsAny(t, "打", "击杀", "消灭", "清怪", "战斗")) return new ParsedCommand(CompanionTaskType.Combat, target, t);
        if (ContainsAny(t, "采", "捡", "收集", "拾取")) return new ParsedCommand(CompanionTaskType.Gather, target, t);
        if (ContainsAny(t, "传送", "去", "前往", "锚点", "神像")) return new ParsedCommand(CompanionTaskType.Teleport, target, t);
    ...
    public static string? ExtractTarget(string text)
    {
        ...
        m = System.Text.RegularExpressions.Regex.Match(text, "(?:去|到|前往|打|击杀|采集|传送|找)([\u4e00-\u9fa5A-Za-z0-9]{2,12})");
